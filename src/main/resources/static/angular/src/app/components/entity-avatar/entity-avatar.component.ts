...
export class EntityAvatarComponent {
  ...
  @Input() entityType!: string; // 'student', 'staff', 'parent'
  ...
  loadAvatar() {
    if (this.entityId && this.organizationId) {
      this.avatarUrl = `/api/attachments/avatar/${this.entityId}?t=${Date.now()}`;
      this.hasAvatar = true;
  ...
  uploadAvatar() {
    ...
    formData.append('userId', this.entityId.toString());
  ...
  deleteAvatar() {
    ...
    this.http.delete(
      `/api/attachments/avatar/${this.entityId}?organizationId=${this.organizationId}`
    ).subscribe({
      next: () => {
