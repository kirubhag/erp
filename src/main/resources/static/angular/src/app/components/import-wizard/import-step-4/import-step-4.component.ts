    ...
    <div class="step-4-container">
      <div class="content-section">
        ...
        <div *ngIf="!isLoading && session">
          ...
          <div class="stats-section">
            <div class="stats-item" *ngIf="getAddedRecords() > 0">
              <span class="stats-label status-added">
                <i class="fas fa-plus-circle me-1"></i>Added
              </span>
              <span class="stats-value ms-2">{{ getAddedRecords() }}</span>
            </div>
            <div class="stats-item" *ngIf="getUpdatedRecords() > 0">
              <span class="stats-label status-updated">
                <i class="fas fa-edit me-1"></i>Updated
              </span>
              <span class="stats-value ms-2">{{ getUpdatedRecords() }}</span>
            </div>
            <div class="stats-item" *ngIf="getSkippedRecords() > 0">
              <span class="stats-label status-skipped">
                <i class="fas fa-skip-forward me-1"></i>Skipped
              </span>
              <span class="stats-value ms-2">{{ getSkippedRecords() }}</span>
            </div>
            <div class="stats-item" *ngIf="hasFailedRecords()">
              <span class="stats-label status-failed">
                <i class="fas fa-times-circle me-1"></i>Failed
              </span>
              <span class="stats-value ms-2">{{ getFailedRecords() }}</span>
            </div>
          </div>          <!-- Success Rate -->
...
export class ImportStep4Component implements OnInit, OnDestroy {
  ...
  getAddedRecords(): number {
    return this.session?.statistics?.addedRecords || 0;
  }

  getUpdatedRecords(): number {
    return this.session?.statistics?.updatedRecords || 0;
  }

  getSkippedRecords(): number {
    return this.session?.statistics?.skippedRecords || 0;
  }

  getFailedRecords(): number {
    return this.session?.statistics?.failedRecords || 0;
  }
