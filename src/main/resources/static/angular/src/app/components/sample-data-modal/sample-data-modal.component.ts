import { Component, OnInit } from '@angular/core';
import { CommonModule } from '@angular/common';
import { FormsModule } from '@angular/forms';
import { Router, ActivatedRoute } from '@angular/router';
import { OrganizationService } from '../../services/organization.service';

interface EntityCheckbox {
  name: string;
  label: string;
  icon: string;
  isImported: boolean;
  isSelected: boolean;
  description: string;
}

@Component({
  selector: 'app-sample-data-modal',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './sample-data-modal.component.html',
  styles: [`
    .sample-data-modal-wrapper {
      position: fixed;
      top: 0;
      left: 0;
      width: 100%;
      height: 100%;
      background-color: rgba(0, 0, 0, 0.7);
      display: flex;
      align-items: center;
      justify-content: center;
      z-index: 2000;
    }
    .modal-overlay {
      position: absolute;
      top: 0;
      left: 0;
      width: 100%;
      height: 100%;
      cursor: default;
    }
    .modal-content {
      position: relative;
      background: white;
      border-radius: 12px;
      box-shadow: 0 20px 60px rgba(0, 0, 0, 0.3);
      max-width: 600px;
      width: 90%;
      max-height: 90vh;
      overflow-y: auto;
      animation: slideUp 0.3s ease-out;
    }
    @keyframes slideUp {
      from {
        opacity: 0;
        transform: translateY(30px);
      }
      to {
        opacity: 1;
        transform: translateY(0);
      }
    }
    .modal-header {
      padding: 30px;
      border-bottom: 1px solid #e9ecef;
      background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);
      color: white;
      border-radius: 12px 12px 0 0;
    }
    .modal-header.success {
      background: linear-gradient(135deg, #28a745 0%, #20c997 100%);
      display: flex;
      flex-direction: column;
      align-items: center;
      text-align: center;
    }
    .check-icon {
      font-size: 3rem;
      margin-bottom: 15px;
    }
    .modal-title {
      font-size: 1.5rem;
      font-weight: 700;
      margin: 0;
    }
    .modal-subtitle {
      font-size: 0.95rem;
      opacity: 0.9;
      margin: 8px 0 0 0;
    }
    .modal-description {
      color: #6c757d;
      margin-bottom: 20px;
    }
    .modal-body {
      padding: 30px;
    }
    .loading-state {
      display: flex;
      flex-direction: column;
      align-items: center;
      justify-content: center;
      padding: 40px 20px;
    }
    .spinner {
      width: 40px;
      height: 40px;
      border: 4px solid #f3f3f3;
      border-top: 4px solid #667eea;
      border-radius: 50%;
      animation: spin 1s linear infinite;
    }
    @keyframes spin {
      0% { transform: rotate(0deg); }
      100% { transform: rotate(360deg); }
    }
    .stats-row {
      display: grid;
      grid-template-columns: repeat(3, 1fr);
      gap: 15px;
      margin-bottom: 25px;
    }
    .stat-box {
      padding: 15px;
      background-color: #f8f9fa;
      border-radius: 8px;
      text-align: center;
    }
    .stat-label {
      font-size: 0.85rem;
      color: #6c757d;
      margin-bottom: 8px;
      text-transform: uppercase;
    }
    .stat-value {
      font-size: 1.8rem;
      font-weight: 700;
      color: #667eea;
    }
    .stat-value.imported {
      color: #28a745;
    }
    .stat-value.selected {
      color: #764ba2;
    }
    .entities-section {
      margin-bottom: 20px;
    }
    .section-header {
      display: flex;
      justify-content: space-between;
      align-items: center;
      margin-bottom: 15px;
    }
    .section-header h3 {
      font-size: 1.1rem;
      font-weight: 600;
      margin: 0;
    }
    .quick-actions {
      display: flex;
      gap: 12px;
    }
    .link-btn {
      background: none;
      border: none;
      color: #667eea;
      cursor: pointer;
      font-size: 0.9rem;
      font-weight: 500;
      padding: 0;
    }
    .link-btn:hover {
      text-decoration: underline;
    }
    .divider {
      color: #ddd;
    }
    .entities-grid {
      display: grid;
      grid-template-columns: repeat(auto-fill, minmax(250px, 1fr));
      gap: 15px;
    }
    .entity-checkbox {
      position: relative;
    }
    .entity-checkbox.disabled {
      opacity: 0.6;
      pointer-events: none;
    }
    .checkbox-container {
      display: flex;
      align-items: flex-start;
    }
    .entity-input {
      width: 20px;
      height: 20px;
      margin-top: 5px;
      margin-right: 12px;
      cursor: pointer;
      flex-shrink: 0;
    }
    .checkbox-label {
      cursor: pointer;
      flex: 1;
    }
    .entity-info {
      padding: 12px;
      border: 1px solid #e9ecef;
      border-radius: 8px;
      background-color: #f8f9fa;
      transition: all 0.3s ease;
    }
    .entity-checkbox:not(.disabled) .entity-input:checked ~ .checkbox-label .entity-info {
      background-color: #e7f3ff;
      border-color: #667eea;
    }
    .entity-header {
      display: flex;
      align-items: center;
      margin-bottom: 6px;
      font-weight: 600;
      color: #333;
    }
    .entity-name {
      flex: 1;
    }
    .badge {
      background-color: #28a745;
      color: white;
      padding: 2px 8px;
      border-radius: 4px;
      font-size: 0.75rem;
      font-weight: 600;
    }
    .entity-description {
      font-size: 0.8rem;
      color: #6c757d;
      margin: 0;
    }
    .info-box {
      padding: 12px;
      background-color: #e7f3ff;
      border-left: 4px solid #667eea;
      border-radius: 6px;
      color: #004085;
      font-size: 0.9rem;
      margin-bottom: 15px;
    }
    .alert {
      padding: 12px;
      border-radius: 6px;
      margin-top: 15px;
    }
    .alert-danger {
      background-color: #f8d7da;
      color: #721c24;
      border: 1px solid #f5c6cb;
    }
    .progress-section {
      text-align: center;
    }
    .progress-section h3 {
      font-size: 1.3rem;
      font-weight: 600;
      margin-bottom: 10px;
    }
    .progress-subtitle {
      color: #6c757d;
      margin-bottom: 25px;
    }
    .progress-bar-container {
      position: relative;
      margin-bottom: 25px;
    }
    .progress-bar-bg {
      height: 8px;
      background-color: #e9ecef;
      border-radius: 4px;
      overflow: hidden;
    }
    .progress-bar-fill {
      height: 100%;
      background: linear-gradient(90deg, #667eea 0%, #764ba2 100%);
      transition: width 0.3s ease;
    }
    .progress-text {
      font-size: 1.2rem;
      font-weight: 700;
      color: #667eea;
      margin-top: 8px;
    }
    .progress-items {
      margin-top: 20px;
      text-align: left;
    }
    .progress-item {
      display: flex;
      align-items: center;
      padding: 10px 0;
      border-bottom: 1px solid #e9ecef;
    }
    .progress-item:last-child {
      border-bottom: none;
    }
    .item-icon {
      width: 40px;
      height: 40px;
      border-radius: 50%;
      background-color: #f8f9fa;
      display: flex;
      align-items: center;
      justify-content: center;
      margin-right: 15px;
      flex-shrink: 0;
      color: #667eea;
    }
    .item-info {
      flex: 1;
    }
    .item-label {
      display: block;
      font-weight: 500;
      color: #333;
    }
    .item-status {
      display: block;
      font-size: 0.85rem;
      color: #667eea;
      margin-top: 2px;
    }
    .item-status.skipped {
      color: #6c757d;
    }
    .success-section {
      text-align: center;
    }
    .success-message {
      padding: 20px;
      background-color: #d4edda;
      border: 1px solid #c3e6cb;
      border-radius: 8px;
      color: #155724;
      margin-bottom: 20px;
      white-space: pre-wrap;
      text-align: left;
      font-size: 0.9rem;
    }
    .redirect-info {
      color: #6c757d;
      margin-top: 20px;
    }
    .dots-loader {
      display: flex;
      justify-content: center;
      gap: 8px;
      margin-top: 15px;
    }
    .dots-loader span {
      width: 10px;
      height: 10px;
      border-radius: 50%;
      background-color: #667eea;
      animation: bounce 1.4s infinite;
    }
    .dots-loader span:nth-child(2) {
      animation-delay: 0.2s;
    }
    .dots-loader span:nth-child(3) {
      animation-delay: 0.4s;
    }
    @keyframes bounce {
      0%, 80%, 100% {
        transform: translateY(0);
        opacity: 1;
      }
      40% {
        transform: translateY(-10px);
        opacity: 0.7;
      }
    }
    .modal-footer {
      padding: 20px 30px;
      border-top: 1px solid #e9ecef;
      display: flex;
      gap: 12px;
      justify-content: flex-end;
      background-color: #f8f9fa;
      border-radius: 0 0 12px 12px;
    }
    .btn {
      padding: 10px 24px;
      border-radius: 6px;
      font-weight: 500;
      border: none;
      cursor: pointer;
      transition: all 0.3s ease;
    }
    .btn-primary {
      background-color: #667eea;
      color: white;
    }
    .btn-primary:hover:not(:disabled) {
      background-color: #5568d3;
    }
    .btn-outline-secondary {
      background-color: white;
      color: #333;
      border: 1px solid #ddd;
    }
    .btn-outline-secondary:hover:not(:disabled) {
      background-color: #f8f9fa;
    }
    .btn:disabled {
      opacity: 0.5;
      cursor: not-allowed;
    }
    @media (max-width: 600px) {
      .modal-content {
        width: 95%;
        border-radius: 12px;
      }
      .modal-body {
        padding: 20px;
      }
      .modal-header {
        padding: 20px;
      }
      .stats-row {
        grid-template-columns: 1fr;
      }
      .entities-grid {
        grid-template-columns: 1fr;
      }
      .modal-footer {
        flex-direction: column-reverse;
      }
      .btn {
        width: 100%;
      }
      .section-header {
        flex-direction: column;
        align-items: flex-start;
        gap: 10px;
      }
      .quick-actions {
        width: 100%;
        justify-content: flex-start;
      }
    }
  `]
})
export class SampleDataModalComponent implements OnInit {
  loading = false;
  populating = false;
  errorMessage = '';
  successMessage = '';
  populationProgress = 0;
  populationStarted = false;
  populationCompleted = false;

  entities: EntityCheckbox[] = [];

  organizationName = '';

  constructor(
    private organizationService: OrganizationService,
    private router: Router,
    private route: ActivatedRoute
  ) { }

  ngOnInit(): void {
    const org = this.organizationService.getCurrentOrganization();
    if (org) {
      this.organizationName = org.name;
    }

    // Load available entities from backend
    this.loading = true;
    this.loadAvailableEntities();
  }

  private loadAvailableEntities(): void {
    this.organizationService.getAvailableEntities().subscribe({
      next: (response: any) => {
        // Map backend entities to our EntityCheckbox format
        this.entities = response.entities.map((entity: any) => ({
          name: entity.name,
          label: entity.description.split('(')[0].trim(), // Use first part as label
          icon: this.getIconForEntity(entity.name),
          isImported: false,
          isSelected: true,
          description: entity.description
        }));
        
        // Check which entities have already been imported
        this.checkImportedEntities();
      },
      error: (error) => {
        console.error('Error loading available entities:', error);
        this.errorMessage = 'Could not load available entities. Please try again.';
        this.loading = false;
      }
    });
  }

  private getIconForEntity(entityName: string): string {
    const iconMap: { [key: string]: string } = {
      'addresses': 'fas fa-map-marker-alt',
      'STUDENTS': 'fas fa-user-graduate',
      'students': 'fas fa-user-graduate',
      'staff': 'fas fa-chalkboard-teacher',
      'parents': 'fas fa-user-friends',
      'subjects': 'fas fa-book',
      'classes': 'fas fa-school',
      'rooms': 'fas fa-door-open',
      'courses': 'fas fa-graduation-cap',
      'timetables': 'fas fa-calendar-alt',
      'attendance': 'fas fa-clipboard-check',
      'grades': 'fas fa-star',
      'exams': 'fas fa-file-alt',
      'finance': 'fas fa-coins',
      'fee_types': 'fas fa-money-bill-wave',
      'fee_structures': 'fas fa-receipt',
      'fee_payments': 'fas fa-credit-card',
      'discount_rules': 'fas fa-tags',
      'fine_categories': 'fas fa-gavel',
      'invoices': 'fas fa-file-invoice',
      'transactions': 'fas fa-exchange-alt',
      'chart_of_accounts': 'fas fa-atlas',
      'journal_entries': 'fas fa-book',
      'budgets': 'fas fa-money-check-alt',
      'scholarships': 'fas fa-award',
      'accounting': 'fas fa-calculator',
      'library': 'fas fa-book-reader',
      'lms': 'fas fa-laptop',
      'tpd': 'fas fa-user-graduate'
    };
    
    return iconMap[entityName] || 'fas fa-database';
  }


  private checkImportedEntities(): void {
    this.organizationService.getImportedEntities().subscribe({
      next: (importedEntities: string[]) => {
        this.entities.forEach(entity => {
          entity.isImported = importedEntities.includes(entity.name);
          // Deselect if already imported
          if (entity.isImported) {
            entity.isSelected = false;
          }
        });
        this.loading = false;
      },
      error: (error) => {
        console.error('Error checking imported entities:', error);
        this.errorMessage = 'Could not check import status. Proceeding with all entities.';
        this.loading = false;
      }
    });
  }

  selectAll(): void {
    this.entities.forEach(entity => {
      if (!entity.isImported) {
        entity.isSelected = true;
      }
    });
  }

  deselectAll(): void {
    this.entities.forEach(entity => {
      entity.isSelected = false;
    });
  }

  isAnySelected(): boolean {
    return this.entities.some(entity => entity.isSelected);
  }

  populateSelectedData(): void {
    const selectedEntities = this.entities
      .filter(entity => entity.isSelected)
      .map(entity => entity.name);

    if (selectedEntities.length === 0) {
      this.errorMessage = 'Please select at least one entity to populate';
      return;
    }

    this.populating = true;
    this.populationStarted = true;
    this.errorMessage = '';
    this.successMessage = '';
    this.populationProgress = 0;

    // Simulate progress updates
    const progressInterval = setInterval(() => {
      if (this.populationProgress < 90) {
        this.populationProgress += Math.random() * 20;
      }
    }, 500);

    this.organizationService.populateSampleData(selectedEntities).subscribe({
      next: (response) => {
        clearInterval(progressInterval);
        this.populationProgress = 100;
        this.populating = false;
        this.populationCompleted = true;

        this.successMessage = `Successfully populated sample data!
        
Total Entities Processed: ${response.totalEntities}
Successful Imports: ${response.successfulImports}
Skipped (Already Imported): ${response.skippedImports}
Failed: ${response.failedImports}

Summary: ${response.summary}`;

        // Redirect to dashboard after 3 seconds
        setTimeout(() => {
          this.router.navigate(['/dashboard']);
        }, 3000);
      },
      error: (error) => {
        clearInterval(progressInterval);
        this.populating = false;
        const errorMsg = error.error?.message || error.message || 'Failed to populate sample data';
        this.errorMessage = errorMsg;
        console.error('Population error:', error);
      }
    });
  }

  /**
   * Populate ALL sample data with correct dependency order.
   * This uses the backend's /populate-all endpoint which ensures
   * proper order: Students -> Subjects -> Grades, etc.
   */
  populateAllData(): void {
    this.populating = true;
    this.populationStarted = true;
    this.errorMessage = '';
    this.successMessage = '';
    this.populationProgress = 0;

    // Select all entities visually
    this.entities.forEach(entity => {
      if (!entity.isImported) {
        entity.isSelected = true;
      }
    });

    // Simulate progress updates
    const progressInterval = setInterval(() => {
      if (this.populationProgress < 90) {
        this.populationProgress += Math.random() * 15;
      }
    }, 600);

    this.organizationService.populateAllSampleData().subscribe({
      next: (response) => {
        clearInterval(progressInterval);
        this.populationProgress = 100;
        this.populating = false;
        this.populationCompleted = true;

        this.successMessage = `Successfully populated ALL sample data!
        
Total Entities Processed: ${response.totalEntities}
Successful Imports: ${response.successfulImports}
Skipped (Already Imported): ${response.skippedImports}
Failed: ${response.failedImports}

Summary: ${response.summary}

Note: Data was loaded in correct dependency order (Students → Subjects → Grades, etc.)`;

        // Redirect to dashboard after 3 seconds
        setTimeout(() => {
          this.router.navigate(['/dashboard']);
        }, 3000);
      },
      error: (error) => {
        clearInterval(progressInterval);
        this.populating = false;
        this.populationStarted = false;
        const errorMsg = error.error?.message || error.message || 'Failed to populate sample data';
        this.errorMessage = errorMsg;
        console.error('Population error:', error);
      }
    });
  }

  skipPopulation(): void {
    // Navigate to dashboard immediately
    this.router.navigate(['/dashboard']);
  }

  getSelectedCount(): number {
    return this.entities.filter(entity => entity.isSelected).length;
  }

  getImportedCount(): number {
    return this.entities.filter(entity => entity.isImported).length;
  }
}
