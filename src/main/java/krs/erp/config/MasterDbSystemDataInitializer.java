...
/**
 * Initializes system-wide data in IAM_MasterDB that should be shared across all tenants.
 * This includes:
                ...
                for (int i = 0; i < fieldNodes.getLength(); i++) {
                    ...
                    if (sectionName != null && !sectionName.isEmpty()) {
                        sectionId = masterJdbcTemplate.queryForObject(
                            "SELECT id FROM IAM_MasterDB.erp_sections WHERE entity_type = ? AND section_name = ?",
        ...
        try {
            ...
            for (int i = 0; i < entityNodes.getLength(); i++) {
                ...
                masterJdbcTemplate.update(
                    ...
                    hasRelTable, icon, route, sequence, presence, isActive, LocalDateTime.now(), LocalDateTime.now(), "SYSTEM", "SYSTEM",
    ...
    private void loadErpEntityRelations() {
        try {
            ...
            for (int i = 0; i < relationNodes.getLength(); i++) {
                ...
                // Get entity IDs from table names (which are used as system_name in erp_entities)
